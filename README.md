...
Migration of Alteryx workflows to Databricks **PySpark** notebooks using **Unity Catalog** and **Delta Lake**. This README documents the **Enterprise Sales Commission** pipeline, the **Rootbeer** medallion pipeline, the shared **`transform.py` API**, and **archived** simple/medium/complex examples (files removed from the repo but kept here as reference).
...
AlteryxMigration/
├── README.md
├── transform.py                              # Enterprise notebooks: shared helpers (%run ./transform)
├── enterprise_explore.py
│
├── Rootbeer-Alteryx-Project/
│   ├── transform.py                          # Rootbeer + macro parity + save_as_uc_table_from_config
...
│   ├── Rootbeer Alteryx Project.yxmd
│   ├── *.csv, *.xlsx (sample sources)
│   └── README.md                             # Workflow inventory + macro → PySpark matrix
│
├── Enterprise_Sales_Commission_Pipeline/     # .yxmd, macros, CSV data
...
**Removed from the repo (reference only below):** `simple_workflow*`, `medium_workflow*`, `complex_workflow*`, `complex_orders.csv` — use git history if you need to recover them.

---

## Rootbeer-Alteryx-Project (Databricks, latest)

| Asset | Purpose |
|-------|---------|
| `notebook_00_setup_configuration.ipynb` | UC catalog/schemas/table names, ADLS `abfss://alteryx@dlsdbxtraining002...`, Volume paths, Spark config, **`pipeline_config`** temp view |
| `notebook_01_rootbeer_transactions_pipeline.ipynb` | Bronze/silver/gold + fact/dim tables; batch macro summary; standard-macro style join |
| `notebook_02_google_trends_union.ipynb` | Union `google_rootbeer_*.csv` into configured tables |
| `Rootbeer-Alteryx-Project/README.md` | Full **`Rootbeer Alteryx Project.yxmd`** tool-container and **macro → PySpark** documentation |

**Run order:** `notebook_00` → `notebook_01` / `notebook_02` (notebooks `%run` config first). Set **`SOURCE_DATA_PATH`** in notebook 00 to your Volume where CSVs land.
...
| Output Delta tables | `managedcatalog.biju_gold.*` (examples — align with your catalog) |
| Rootbeer pipeline data | Configured in `notebook_00` (`SOURCE_DATA_PATH` on ADLS-backed Volume) |
...
| Location | Used by |
|----------|---------|
| **Repository root** `transform.py` | `enterprise_01` … `enterprise_06` |
| **`Rootbeer-Alteryx-Project/transform.py`** | Rootbeer `.ipynb` notebooks; extends helpers with **`build_rootbeer_enriched_transactions`**, macro parity functions, **`save_as_uc_table_from_config`**, etc. |

---

## transform.py — Shared Utility Library (repository root)

The root `transform.py` is a Databricks notebook loaded at the top of **enterprise** workflow notebooks via:

```python
%run ./transform
```

This makes all helper functions below available in the calling notebook's scope without any imports. Every function logs its operation and row counts to the cell output for observability. **Rootbeer** notebooks use **`%run ./transform`** from inside **`Rootbeer-Alteryx-Project/`** (the extended library there).
...
#### `save_as_uc_table_from_config(df, cfg, schema_key=..., table_key=...)` *(Rootbeer `transform.py` only)*

Builds the full three-level name `catalog.schema.table` from a **`pipeline_config`** row (notebook 00) and runs **`CREATE OR REPLACE TABLE … AS SELECT`**. Escapes identifiers for SQL (e.g. catalogs with hyphens).
...
### Archived: simple / medium / complex workflows *(files no longer in repo)*

The following three subsections document **simple_workflow**, **medium_workflow**, and **complex_workflow** as they existed when those paths were in the repository. They are **kept for reference** (patterns map to `transform.py`). To recover files, use: `git log --all --full-history -- "**/simple_workflow*"`. The active maintained sample is **`Rootbeer-Alteryx-Project`**.
...
| — | `save_as_uc_table_from_config()` | Rootbeer only: Delta table from `pipeline_config` keys |
...
### Rootbeer pipeline (`Rootbeer-Alteryx-Project/`)

Upload **`Rootbeer-Alteryx-Project/`** (including **`transform.py`** and the three `.ipynb` files) to a Databricks folder. Run **`notebook_00_setup_configuration.ipynb`** first so **`pipeline_config`** exists, then **`notebook_01`** and/or **`notebook_02`**. Table names and paths come from notebook 00 (ADLS account **`dlsdbxtraining002`**, container **`alteryx`** in the default template).

### Simple / Medium / Complex workflows *(archived — notebooks removed from repo)*

If you restore them from git history, they are independent. Example queries against the old gold tables:
